...
[source,bash]
----
instance_namespace="your-namespace"
instance_name="the name of RedisInstance resource in $instance_namespace"
----
...
[source,bash]
----
redis_composition=$(kubectl -n $instance_namespace get RedisInstance $instance_name -o jsonpath="{.spec.resourceRef.name}")
...
snapshot_id="" <1>
----
<1> Selected long snapshot ID
+
[TIP]
====
If you just need the latest snapshot, use this command:
[source,bash]
----
snapshot_id=$(restic snapshots --latest 1 --json | jq -r '.[].id')
----
====
...
[source,bash]
----
kubectl -n $redis_namespace apply -f - <<EOF
include::example$restore-k8up.yaml[]
EOF
----

. Mount the restic repository locally
+
[source,bash]
----
mkdir -p restic_mount restic_restore
restic mount restic_mount/ > restic_mount.log 2>&1 &
restic_pid=$!
----

. Select the snapshot and extract the data
+
[NOTE]
====
The latest snapshot is used in the snippet below, but you can choose a specific date by directory name by running `ls restic_mount/snapshots`.
====
+
[source,bash]
----
snapshot=latest
tar -zf "restic_mount/snapshots/$snapshot/sv-redis-$redis_composition-redis.tar.gz" -C restic_restore/
----
...
kubectl cp restic_restore/data $redis_namespace/redis-restore:/
...
[source,bash]
----
kill $minio_pfid $restic_pid
rm -rf minio_portforward.log restic_mount.log restic_mount restic_restore
